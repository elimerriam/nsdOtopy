...
%% CONSTRAINED RESIDUAL preferred ORIENTATION
isubplot=isubplot+1;
subplot(rows,cols, isubplot);
plotOriLines(allResidOri{iroi}(isplit,:), allPrfX{iroi}, allPrfY{iroi}, allPrfEcc{iroi},(0.4*allNsdR2{iroi}(isplit,:)));
plotOriLines(allResidOri{iroi}(isplit,:), allPrfX{iroi}, allPrfY{iroi}, allPrfEcc{iroi},(3*allNsdR2{iroi}(isplit,:)));

xlabel('\itx position (deg)');
ylabel('\ity position (deg)');

%% CONSTRAINED PREDICTED preferred ORIENTATION
isubplot=isubplot+1;
subplot(rows,cols, isubplot);
plotOriLines(allPredOri{iroi}(isplit,:), allPrfX{iroi}, allPrfY{iroi}, allPrfEcc{iroi},(0.4*allNsdR2{iroi}(isplit,:)));
plotOriLines(allPredOri{iroi}(isplit,:), allPrfX{iroi}, allPrfY{iroi}, allPrfEcc{iroi},(3*allNsdR2{iroi}(isplit,:)));
...
for iori=1:length(oris)
    drawOriLine(oriX(iori), oriY(iori), pi/2-oris(iori), linelength, linewidth, cMap(1+floor((mod(oris(iori),pi))*255/(pi)),:));
    hold on
end
xlim([-0.5 2]);
ylim([-2 0.5]);
...
%%
set(gcf,'position',[150 180 3*250 210]);
h.Units = 'centimeters'; 
h.PaperSize=[10 3.5]; 
if toSavePdf
    print('-painters','-dpdf',[figFolder 'fig5_top']);
end
